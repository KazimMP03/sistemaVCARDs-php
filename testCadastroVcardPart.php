...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($checkChaveResult) > 0) {
        ...
        if ($result) {
            echo "<script>alert('Vcard Cadastrado Com Sucesso!'); window.location.href='sistPart.php?page=consultavcardsPart.php';</script>";
            exit;
        } else {
            // Handle the case where the insertion failed
            echo "<script>alert('Erro ao cadastrar Vcard!'); window.location.href='sistPart.php?page=consultavcardsPart.php';</script>";
            exit;
        }
    } else {
        ...
        echo "window.location.href='sistPart.php?page=cadastrovcardPart.php?$urlParams';</script>";
        exit;
    }
}

echo "<script>alert('ERRO!'); window.location.href='sistPart.php?page=consultavcardsPart.php';</script>";
exit;
